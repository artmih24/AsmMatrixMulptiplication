...
int AsmMatrixMulBlockV6(float *At, float *B, float *C, int sizeM, int sizeN, int sizeK) {
    int i = 0,
        ...
        n = 0,
        blockSize_min = 8,
        blockSizeMmax = 2 * 4096 / (sqrt(sizeN * sizeK)),
        //2 * 4096 / ((sizeN + sizeK) / 2),
        //4 * 4096 / ((sizeN + sizeK) / 2),
        //(4096 / ((sizeN + sizeK) / 2) > blockSize_min) ? 4096 / ((sizeN + sizeK) / 2) : blockSize_min,
        //4096 / ((sizeN + sizeK) / 2),
        //128,
        blockSizeNmax = 4096 / 2 * (sqrt(sizeM * sizeK)),
        //4096 / 2 * ((sizeM + sizeK) / 2),
        //4096 / ((sizeM + sizeK) / 2),    
        //(4096 / ((sizeM + sizeK) / 2) > blockSize_min) ? 4096 / ((sizeM + sizeK) / 2) : blockSize_min,
        //4096 / ((sizeM + sizeK) / 2),
        //16, // 32,
        blockSizeKmax = 4 * 4096 / (sqrt(sizeM * sizeN)),
        //4 * 4096 / ((sizeM + sizeN) / 2),
        //4 * 4096 / ((sizeM + sizeN) / 2),
        //(4096 / ((sizeM + sizeN) / 2) > blockSize_min) ? 4096 / ((sizeM + sizeN) / 2) : blockSize_min,
        //4096 / ((sizeM + sizeN) / 2),
        //128,
        blockSizeM = (sizeM < blockSizeMmax) ? sizeM : (blockSizeMmax > blockSize_min) ? blockSizeMmax : blockSize_min,
        //(sizeM < blockSizeMmax) ? sizeM : blockSizeMmax, 
        //(sizeM < blockSizeMmax) ? sizeM : (blockSizeMmax > blockSize_min) ? blockSizeMmax : blockSize_min,
        blockSizeN = (sizeN < blockSizeNmax) ? sizeN : (blockSizeNmax > blockSize_min) ? blockSizeNmax : blockSize_min,
        //(sizeN < blockSizeNmax) ? sizeN : blockSizeNmax, 
        //(sizeN < blockSizeNmax) ? sizeN : (blockSizeNmax > blockSize_min) ? blockSizeNmax : blockSize_min,
        blockSizeK = (sizeK < blockSizeKmax) ? sizeK : (blockSizeKmax > blockSize_min) ? blockSizeKmax : blockSize_min,
        //(sizeK < blockSizeKmax) ? sizeK : blockSizeKmax, 
        //(sizeK < blockSizeKmax) ? sizeK : (blockSizeKmax > blockSize_min) ? blockSizeKmax : blockSize_min,
        offsetM = 0,
        offsetN = 0,
        offsetK = 0;      
    float *fragAt = 0,
